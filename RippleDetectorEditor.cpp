...
CustomTextBoxParameterEditor::CustomTextBoxParameterEditor(Parameter *param)
  ...
  parameterNameLabel =
      std::make_unique<Label>("Parameter name", param->getName());
  ...
  if (param->getType() == Parameter::FLOAT_PARAM)
    valueTextBox = std::make_unique<Label>("Parameter value",
                                           String(float(param->getValue())));
  else
    valueTextBox = std::make_unique<Label>("Parameter value",
                                           param->getValue().toString());

  valueTextBox->setFont(Font("CP Mono", "Plain", 15));
  valueTextBox->setName(param->getProcessor()->getName() + " (" +
                        String(param->getProcessor()->getNodeId()) + ") - " +
                        param->getName());
...
void CustomTextBoxParameterEditor::updateView() {
  ...
  if (param != nullptr) {
    ...
    if (param->getType() == Parameter::FLOAT_PARAM)
      valueTextBox->setText(String(float(param->getValue())),
...
void CustomTextBoxParameterEditor::resized() {
  ...
  valueTextBox->setBounds(int(3 * finalWidth / 5), 0, int(2 * finalWidth / 5),
                          18);
}
...
RippleDetectorEditor::RippleDetectorEditor(GenericProcessor *parentNode)
  ...
  addComboBoxParameterEditor("Ripple_Out", 10, 50);
  ...
  calibrateButton->setBounds(10, 100, 80, 25);
